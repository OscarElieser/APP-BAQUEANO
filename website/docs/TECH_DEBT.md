...
Documentar de forma explícita y transparente aquellas mejoras arquitectónicas, refactorizaciones o dependencias de infraestructura externa que deben gestionarse metódicamente para evitar degradaciones en escalabilidad, seguridad o rendimiento con el crecimiento de la plataforma.
...
- **Problema**: El listado de destinos actualmente obtiene documentos mediante consultas con límite estático. A medida que el catálogo nacional supere los cientos de destinos, la consulta en bloque consumirá lecturas innecesarias.
...
- **Prioridad**: Alta.
- **Solución Recomendada**: Desplegar una Cloud Function que genere variantes automáticas (`thumb_300`, `card_800`, `hero_1600`) en formato WebP con compresión optimizada.
...
- **Prioridad**: Alta.
- **Solución Recomendada**: Script CLI seguro en `scripts/set-admin-claims.js` que reciba el Service Account privado para emitir los claims oficiales.
...
### Ítem 5: Telemetría RUM en Conexiones Móviles Rurales
- **Problema**: Las pruebas de Core Web Vitals actuales se basan en mediciones sintéticas en laboratorio.
- **Impacto**: Podría haber diferencias en tiempos de respuesta reales en zonas con conectividad 3G/Edge (ej. Río San Juan, Bosawás).
- **Prioridad**: Media.
- **Solución Recomendada**: Conectar un script ligero de Web Vitals (`web-vitals` library) que reporte métricas percentil 75 (p75) al endpoint de analítica.
