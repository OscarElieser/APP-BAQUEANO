...
(function (window, document) {
  ...
  const ACTIONS = new Set(['open_destination','open_map','show_place','search_places','build_itinerary','save_favorite','show_nearby','show_emergency','open_booking','open_route']);
  ...
  const state = { open: false, busy: false, minimized: false, dragging: false, controller: null, recognition: null, timers: [], lastActivity: Date.now() };
    ...
    root.innerHTML = `
      ...
      <aside class="bq-drawer" id="bqDrawer" aria-hidden="true" aria-label="Baqueano Digital">
        ...
        <div class="bq-quick" aria-label="Acciones rápidas">
          <button data-quick="build_itinerary">🗺️ Planificar viaje</button><button data-quick="search_places">🌋 Descubrir destinos</button><button data-quick="lodging">🏨 Hospedaje</button><button data-quick="food">🍽️ Dónde comer</button><button data-quick="experiences">🥾 Aventuras</button><button data-quick="show_nearby">📍 Qué hay cerca</button><button data-quick="favorites">❤️ Favoritos</button><button data-quick="show_emergency">🚨 SOS 24/7</button><button data-quick="country">🇳🇮 Conocer Nicaragua</button><button data-quick="surprise">✨ Sorpréndeme</button>
        </div>
  ...
  function context() {
    ...
    return {
      ...
      filters: { active: activeFilters }, recentItems: safeJson(sessionStorage.getItem('baqueano_recent_items'), []).slice(-5), tripProfile: session.tripProfile
    };
  }
  ...
  async function streamText(text) {
    ...
    if (preferences.voice && text) speak(text);
  }

  function speak(text) { if (!('speechSynthesis' in window) || !preferences.voice) return; speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(String(text).slice(0, 1200)); utterance.lang = 'es-NI'; utterance.rate = .96; speechSynthesis.speak(utterance); }
  ...
  async function ask(raw) {
    ...
    $('#bqMessages').setAttribute('aria-busy', 'true'); $('[data-command="stop"]').hidden = false; $('#bqMascot').classList.add('is-thinking');
    ...
    try {
      ...
      thinking?.closest('article')?.remove(); session.tripProfile = Object.assign({}, session.tripProfile, data.tripProfilePatch || {}); await streamText(data.message); renderActions(data.actions || []); track('assistant_response', { mode: data.mode, latency_ms: Math.round(performance.now() - started) });
    } catch (error) {
    ...
    } finally { state.busy = false; state.controller = null; $('#bqMessages').setAttribute('aria-busy', 'false'); $('[data-command="stop"]').hidden = true; $('#bqMascot').classList.remove('is-thinking'); }
  }
  ...
  function executeAction(action) {
    ...
    const routes = { open_map: 'destinos.html#mapa', show_place: 'destinos.html', search_places: 'destinos.html', open_destination: 'destinos.html', build_itinerary: 'baqueano-ai.html#planner', show_emergency: 'index.html#sos', open_route: 'index.html#routeBuilderSection', open_booking: 'mi-negocio.html' };
  ...
  function hide() { session.hidden = true; saveSession(); root.hidden = true; close(); track('assistant_hidden'); }
  function showSuggestion(text) { if (state.open || session.hidden || isSensitiveInteraction()) return; const box = $('#bqSuggestion'); $('p', box).textContent = text; box.hidden = false; root.classList.add('is-waving'); setTimeout(() => { root.classList.remove('is-waving'); hideSuggestion(); }, 8500); track('assistant_shown'); }
  ...
  function startRecognition() {
    ...
    recognition.onerror = () => { $('#bqInput').placeholder = 'No pude escuchar. Intentá nuevamente.'; }; recognition.onend = () => { state.recognition = null; }; recognition.start();
  }
  ...
  root.addEventListener('click', event => {
    ...
    if (quick) { track('assistant_quick_action', { action: quick }); if (ACTIONS.has(quick)) return executeAction({ type: quick }); const prompts = { lodging: 'Busco hospedaje con información registrada.', food: '¿Dónde puedo comer comida local?', experiences: 'Mostrame aventuras y experiencias.', favorites: 'Quiero ver mis favoritos.', country: 'Quiero conocer Nicaragua.', surprise: 'Sorpréndeme con un destino verificado.' }; return ask(prompts[quick]); }
