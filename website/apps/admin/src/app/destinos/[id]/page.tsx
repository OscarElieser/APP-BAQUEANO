  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        else setError("No se encontrÃ³ el destino.");
      } catch (err) {
    ...
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {place.status === "draft" && (
        <div className="bg-[#165D6F]/20 border border-[#165D6F]/50 text-slate-200 p-4 rounded-xl text-sm flex items-start gap-3">
          <div className="text-[#F65E01] mt-0.5">âœ¨</div>
          <p>
            <strong className="text-white">Tip de CuratorÃ­a:</strong> Abre el <strong>Copiloto IA</strong> y pide una evaluaciÃ³n SEO y cultural para este destino. El orquestador analizarÃ¡ sus metadatos e imÃ¡genes para sugerir mejoras antes de publicarlo.
          </p>
        </div>
      )}
      ...
      <div className="bg-[#1e293b]/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-2xl flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          <h3 className="text-sm font-semibold text-slate-400 tracking-wider uppercase flex items-center gap-2">
            <ImageIcon className="w-4 h-4" /> GalerÃ­a Visual
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* UBICACIÃ“N */}
          <div className="flex flex-col gap-4">
            <h3 className="text-sm font-semibold text-slate-400 tracking-wider uppercase">UbicaciÃ³n (GIS)</h3>
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3 text-slate-300">
                <MapPin className="w-4 h-4 text-[#F65E01]" />
                <span className="text-sm">{place.departmentName}, {place.municipalityName}</span>
              </div>
              <div className="flex items-center gap-3 text-slate-300">
                <Compass className="w-4 h-4 text-slate-500" />
                <span className="text-sm font-mono text-slate-400">Lat: {place.latitude} / Lng: {place.longitude}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3 text-slate-300">
                ...
                <span className="text-sm capitalize">CategorÃ­a: {place.categoryName.replace(/_/g, " ")}</span>
              </div>
              <div className="flex items-center gap-3 text-slate-300">
