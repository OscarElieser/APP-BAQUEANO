      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm md:text-base">
            Administra los territorios, polÃ­gonos y lugares geogrÃ¡ficos de la plataforma Baqueano.
          </p>
        </div>
      ...
      {!result.isConnected && (
        <div className="bg-orange-500/10 border border-orange-500/20 text-orange-400 p-4 rounded-xl flex items-center gap-3 backdrop-blur-md">
          ...
          <p className="text-sm font-medium">
            AtenciÃ³n: El sistema estÃ¡ operando en modo local (sin conexiÃ³n a Firebase). {result.warning}
          </p>
        </div>
      )}
        ...
        <AdminTable>
          <AdminTableHeader>
            <AdminTableRow>
              ...
              <AdminTableHead>CategorÃ­a</AdminTableHead>
              <AdminTableHead>UbicaciÃ³n</AdminTableHead>
          ...
          <AdminTableBody>
            {destinos.length === 0 ? (
              ...
              destinos.map((destino: PlaceRecord) => {
                ...
                return (
                  <AdminTableRow key={destino.placeId} className="group hover:bg-slate-800/50 transition-colors">
                    <AdminTableCell className="font-medium text-slate-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-slate-400 font-mono truncate max-w-[150px]">ID: {destino.placeId.split('-')[0]}</span>
                        </div>
                      </div>
                    </AdminTableCell>
                    <AdminTableCell className="text-slate-300">
                      <span className="capitalize">{destino.categoryName.replace(/_/g, " ")}</span>
                    </AdminTableCell>
                    <AdminTableCell className="text-slate-300 flex flex-col">
                      <span className="text-sm">{destino.departmentName}</span>
                      <span className="text-xs text-slate-500">{destino.latitude.toFixed(4)}, {destino.longitude.toFixed(4)}</span>
                    </AdminTableCell>
                    <AdminTableCell>
                    ...
                    <AdminTableCell className="text-right">
                      <Link href={`/destinos/${destino.placeId}`}>
