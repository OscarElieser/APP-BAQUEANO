...
export function generateStaticParams() {
  return featuredDestinations.map((destination) => ({ slug: destination.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const destination = featuredDestinations.find((item) => item.slug === slug);
  return {
    title: destination?.name ?? "Destino",
    description: destination?.summary,
    openGraph: { images: destination ? [destination.heroImage] : [] }
  };
}
  ...
  return (
    <main className="pb-20">
      <section className="relative min-h-[78vh] overflow-hidden px-4 pb-10 pt-32 sm:px-6 lg:px-8">
        <Image src={destination.heroImage} alt={destination.name} fill priority sizes="100vw" className="object-cover" />
        ...
        <div className="relative z-10 mx-auto flex min-h-[62vh] max-w-7xl flex-col justify-end">
          ...
          <p className="mt-5 flex items-center gap-2 font-tech text-sm uppercase text-[#F4E6C1]"><MapPin size={16} /> {destination.department}, {destination.municipality}</p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 pt-10 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
        <article className="glass-panel p-6 md:p-8">
          <p className="text-lg leading-8 text-white/78">{destination.story}</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <MetricTile label="Duracion" value={`${destination.durationHours}h`} icon={<Clock size={20} />} />
            <MetricTile label="Desde" value={`$${destination.priceUsd}`} icon={<Calendar size={20} />} />
            <MetricTile label="Impacto" value={`${destination.sustainabilityScore}%`} icon={<Heart size={20} />} />
          </div>
        ...
        <aside className="glass-panel h-fit p-5">
          ...
          <p className="mt-2 font-tech text-xl font-black text-white">{destination.coordinates.latitude.toFixed(4)} N</p>
          <p className="font-tech text-xl font-black text-white">{Math.abs(destination.coordinates.longitude).toFixed(4)} W</p>
          <div className="mt-6 grid gap-3">
            <BaqueanoButton>Reservar</BaqueanoButton>
            <BaqueanoButton variant="ghost"><Heart size={16} /> Guardar</BaqueanoButton>
