 ...
 * QUÉ: sanitización, error honesto de indisponibilidad y contrato de acciones.
 */
"use strict";
const assert = require("node:assert/strict");
const test = require("node:test");
const {cleanText, createAiChatService, sanitizeActions, sanitizeTripProfilePatch} = require("../lib/ai-chat");
...
test("indisponibilidad de IA nunca genera una respuesta simulada", async () => { const db = {collection: () => ({limit: () => ({get: async () => ({docs: []})})})}; const service = createAiChatService({db, getApiKey: () => "", fetchImpl: async () => { throw new Error("network"); }}); const result = await service({ip: "test-no-fallback", body: {message: "Quiero un mapa"}}); assert.equal(result.status, 503); assert.equal(result.body.ok, false); assert.equal(result.body.error.code, "AI_UNAVAILABLE"); assert.equal(result.body.message, undefined); });
